...
#include "data.h"
#include "onnx_elu_mem_layout.h"
#include "onnx_elu_params.h"
#include "onnx_elu_task_bin.h"

static int init_data(void *params)
{
    uint32_t offset;
    volatile onnx_elu_params_t *elu_params;

    elu_params = (volatile onnx_elu_params_t *) params;
    for (int i = 0; i < LEN; i++) {
    ...
    elu_params->add_alpha = ALPHA_BASE;
    elu_params->addr_src = INPUT_BASE;
    elu_params->addr_res = RES_BASE;
    elu_params->addr_exp = EXP_BASE;
    elu_params->len = LEN;
...
static int run_spatz_task()
{
    ...
    spatz_run_task_with_params(ONNX_ELU_TASK, ONNX_ELU_PARAMS_BASE);
...
static bool check_result(void *params)
{
    volatile onnx_elu_params_t *elu_params;
    elu_params = (volatile onnx_elu_params_t *) params;
    return vector_compare_fp16_bitwise(elu_params->addr_res, elu_params->addr_exp, elu_params->len);
}
...
static bool run_test()
{
    ...
    params = (volatile onnx_elu_params_t *) ONNX_ELU_PARAMS_BASE;
