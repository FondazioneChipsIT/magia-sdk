...
static int init_input_params(void *params, const float16 *X, const float16 *W, const float16 *B)
{
    ...
    total_in = conv_params->n_batches * (conv_params->c_in_g * groups) * conv_params->h_in *
               conv_params->w_in;
    total_w =
        conv_params->c_out * conv_params->c_in_g * conv_params->kernel_h * conv_params->kernel_w;
    ...
    if (conv_params->has_bias) {
        idma_memcpy_1d(
            &idma_ctrl, 0, (uint32_t)B, (uint32_t)conv_params->shard_B, c_out * sizeof(float16));
        eu_idma_wait_a2o(&eu_ctrl, WFE);
    } else {
...
void MAGIA_conv2d_fp16_spatz(const float16 *X,
                             const float16 *W,
                             const float16 *B,
                             float16 *Y,
                             uint32_t input_shape[4],
                             ...
                             int has_bias)
{
    ...
    PROF_PHASE(prof_cyc_data_in, ret, init_input_params(params, X, W, B));
    if (ret != 0) {
