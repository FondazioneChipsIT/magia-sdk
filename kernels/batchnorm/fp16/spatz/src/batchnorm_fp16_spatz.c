...
static int init_input_params(void *params,
                             ...
                             const float16 *scale,
                             const float16 *B,
                             const float16 *input_mean,
                             const float16 *input_var,
...
void MAGIA_batchnorm_fp16_spatz(const float16 *X,
                                const float16 *scale,
                                const float16 *B,
                                const float16 *input_mean,
                                const float16 *input_var,
                                ...
                                uint32_t shape[4])
{
    ...
    PROF_PHASE(prof_cyc_alloc, ret, alloc_l1(&params, shape));
    if (ret != 0) {
    ...
    PROF_PHASE(prof_cyc_data_in,
               ret,
               init_input_params(params, X, scale, B, input_mean, input_var, epsilon));
    if (ret != 0) {
