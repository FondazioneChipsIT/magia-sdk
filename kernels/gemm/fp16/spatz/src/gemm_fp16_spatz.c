                           ...
                           float16 *Y)
{
    ...
    PROF_PHASE(
        prof_cyc_alloc, ret, alloc_l1(&params, A_shape, B_shape, C_shape, Y_shape, transA, transB));
    if (ret != 0) {
    ...
    PROF_PHASE(prof_cyc_data_in,
               ret,
               init_input_params(params, A, B, C, alpha, beta, A_shape, B_shape, C_shape, Y_shape));
    if (ret != 0) {
