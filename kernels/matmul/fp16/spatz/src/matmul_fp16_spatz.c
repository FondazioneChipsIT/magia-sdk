...
void MAGIA_matmul_fp16_spatz(const float16 *A,
                             const float16 *B,
                             float16 *Y,
                             uint32_t M,
                             uint32_t K,
                             ...
                             uint32_t b_batched)
{
    ...
    PROF_PHASE(prof_cyc_alloc, ret, alloc_l1((void **)&params, M, K, O, total_batches));
    if (ret != 0) {
    ...
    PROF_PHASE(
        prof_cyc_data_in, ret, init_input_params((void *)params, A, B, a_batched, b_batched));
    if (ret != 0) {
