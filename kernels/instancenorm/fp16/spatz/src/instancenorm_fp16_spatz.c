                                   ...
                                   uint32_t input_shape[4])
{
    ...
    PROF_PHASE(prof_cyc_alloc, ret, alloc_l1((void **)&params, input_shape, &num_channels));
    if (ret != 0) {
        printf("[CV32 (%d)] [%s] L1 allocation failed with error: %d\n", HID, KERNEL_NAME, ret);
        return;
    }

    PROF_PHASE(prof_cyc_data_in, ret, init_input_params((void *)params, input, scale, B, epsilon));
    if (ret != 0) {
