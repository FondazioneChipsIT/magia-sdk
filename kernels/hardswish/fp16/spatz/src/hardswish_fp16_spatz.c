...
#include "hardswish_fp16_spatz_task_bin.h"

#define HID         get_hartid()
#define KERNEL_NAME "hardswish_fp16_spatz"

static int alloc_l1(void **params, uint32_t size)
{
    volatile hardswish_fp16_spatz_params_t *hardswish_params;
    uintptr_t shard_X;
    uintptr_t shard_Y;
    ...
    hardswish_params = l1_alloc(sizeof(hardswish_fp16_spatz_params_t));
    if (!hardswish_params)
        return ENOMEM;
    ...
    hardswish_params->shard_X = shard_X;
    hardswish_params->shard_Y = shard_Y;
    hardswish_params->start   = start;
    hardswish_params->len     = len;
    hardswish_params->end     = end;

    *params = (void *)hardswish_params;
...
static int init_input_params(void *params, const float16 *X)
{
    ...
    hardswish_params = (volatile hardswish_fp16_spatz_params_t *)params;
    start            = hardswish_params->start;
    len              = hardswish_params->len;
    ...
    /* This tile's slice [start, start+len) is contiguous in L2. The Spatz task writes every
       output (Y = hardswish(X)), so shard_Y is not zeroed here. */
    idma_memcpy_1d(&idma_ctrl,
                   0,
                   (uint32_t)(X + start),
                   (uint32_t)hardswish_params->shard_X,
                   len * sizeof(float16));
...
static int offload_spatz_task(void *params)
{
    ...
    eu_ctrl_init(&eu_ctrl);
    spatz_run_task_with_params(HARDSWISH_FP16_SPATZ_TASK, (uint32_t)params);
...
static int store_result(void *params, float16 *dst)
{
    ...
    idma_memcpy_1d(&idma_ctrl,
                   1,
                   (uint32_t)(dst + start),
                   (uint32_t)hardswish_params->shard_Y,
                   len * sizeof(float16));
...
void MAGIA_hardswish_fp16_spatz(const float16 *X, float16 *Y, uint32_t size)
{
    int ret;
